...
function renderCart() {
  ...
  if (cartArray.length === 0) {
    tbody.innerHTML = `<tr><td colspan="6" class="text-center py-4 text-red-500">Your cart is empty</td></tr>`;
    totalElement.textContent = "Total: $0";
    updateCartCount();
    return;
  }
  ...
  cartArray.forEach((item, index) => {
    ...
    tr.innerHTML = `
      ...
      <td class="px-4 py-2 text-center border border-gray-200">${item.name.slice(0, 25)}</td>
      <td class="px-4 py-2 text-center border border-gray-200">$${item.price.toFixed(2)}</td>
      
      <!-- ✅ Quantity Column -->
      <td class="px-4 py-2 text-center border border-gray-200">
        <div class="flex items-center justify-center space-x-2">
          <button class="decrement bg-gray-300 hover:bg-gray-400 px-2 rounded" data-index="${index}">-</button>
          <span class="font-semibold">${quantity}</span>
          <button class="increment bg-gray-300 hover:bg-gray-400 px-2 rounded" data-index="${index}">+</button>
        </div>
      </td>

      <td class="px-4 py-2 text-center border border-gray-200">$${subTotal.toFixed(2)}</td>
      <td class="px-4 py-2 text-center border border-gray-200">
        <button class="hover:scale-105 transition-all duration-300 bg-[#fb2c36] text-white text-center py-1 px-3 rounded removeBtn" data-index="${index}">
          Remove
        </button>
